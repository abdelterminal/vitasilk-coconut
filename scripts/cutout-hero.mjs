...
//
// KEEP THIS METHOD FOR THE COCONUT BOTTLE. The three failures above were
// measured against a beige bottle on a cream ground, and a white bottle on the
// pale grey studio ground this SKU is shot on is the same problem or worse —
// failures 2 and 3 in particular get harder, since a white body overlaps the
// background in luminance almost completely and carries no chroma to separate
// on. The blue sibling could get away with a plain neighbour-step flood because
// a saturated royal-blue body steps hard against white; nothing here does.
//
// The parts to watch on this specific bottle, in order of how often they break:
//   - the BLACK PUMP is the one high-contrast feature and it cuts cleanly, but
//     the fill can walk into it from above if the ring breaks at the collar;
//   - the SILVER BASE BAND is the real risk. It is a mid-grey ring sitting on a
//     grey ground, the lowest-contrast boundary in the frame. Check it at 100%
//     in _cutout-check.png before accepting a run;
//   - the WHITE SHOULDER, where the body curves away and its shaded edge lands
//     within a few levels of the background.
//
...
// immediately in _cutout-check.png as a bite out of the silhouette). On a white
// bottle expect to reach for a LOWER EDGE than the default before a higher one.
//
// Run with: node scripts/cutout-hero.mjs [srcPath]
import sharp from "sharp";

const SRC = process.argv[2] ?? "C:/Users/Brandshift 01/Downloads/vitasilk coconut img/bottle-source.png";
